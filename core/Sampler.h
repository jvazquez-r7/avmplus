...
namespace avmplus
{
    ...
    public:
        ...
        // Called when a ScriptObject based object has been reallocated (a back buffer it owns has grown).
        // The object passed in is the dependent object for the ScriptObject. It can be of any type.
        virtual void recordObjectReallocation(const void *obj) = 0;

        // Call to add a dependent object. A dependent object is one that should be watched for
        // reallocations in recordObjectReallocation(). A dependent object can be of any type.
        virtual void addDependentObject(const avmplus::AvmPlusScriptableObject *obj, const void *depObj) = 0;

        // Called when a new object of the given type has been allocated. For generic recording of any object type.
        virtual void recordNewObjectAllocation(void *obj, const char *objectType, size_t size) = 0;
    ...
    // Get the current memory sampler. Global utility function, should only be used when we
    // don't already have a reference to the current GC. If the GC is known, then GetAttachedSampler
    // should be called on the GC object.
    ...
    public:
        ...
        void addDependentObject(const avmplus::AvmPlusScriptableObject *, const void *) {};
