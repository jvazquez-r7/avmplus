...
shell_suffix="_64"
shell_extension=""
base_configure_args="--target=x86_64-linux"
...
cd $WS/objdir/shell/
wget ${build_url}platform=linux64/artifact/objdir/shell/avmshell${shell_suffix}${shell_extension}
chmod 777 avmshell${shell_suffix}${shell_extension}
wget ${build_url}platform=linux64/artifact/objdir/shell/avmshell_s${shell_suffix}${shell_extension}
chmod 777 avmshell_s${shell_suffix}${shell_extension}
wget ${build_url}platform=linux64/artifact/objdir/shell/avmshell_d${shell_suffix}${shell_extension}
chmod 777 avmshell_d${shell_suffix}${shell_extension}
wget ${build_url}platform=linux64/artifact/objdir/shell/avmshell_sd${shell_suffix}${shell_extension}
...
#############################
# JIT or Die Release Acceptance
#############################
export shell_name=avmshell
export AVM="$WS/objdir/shell/$shell_name$shell_suffix"
export mode="release-jitordie"
export vmargs="-Djitordie"
cd $WS/build/buildbot/slaves/scripts
../all/run-acceptance-generic.sh "$rev_id" "$shell_name$shell_suffix" "$vmargs" "" "--showtimes --log runtests-$mode.txt --logjunit=acceptance-$mode.xml --threads=$threads --testtimeout=300 $suite"
failures=`grep "^failures" $WS/test/acceptance/runtests-$mode.txt | awk '{print $3}'`
if [ "$failures" = "0" ]; then
...
#############################
# NoDebugger ReleaseDebugger Acceptance
#############################
export shell_name=avmshell_s
export AVM="$WS/objdir/shell/$shell_name$shell_suffix"
export mode="release-nodebugger"
export vmargs="-Dnodebugger"
cd $WS/build/buildbot/slaves/scripts
../all/run-acceptance-generic.sh "$rev_id" "$shell_name$shell_suffix" "$vmargs" "" "--showtimes --log runtests-$mode.txt --logjunit=acceptance-$mode.xml --threads=$threads --testtimeout=300 $suite"
failures=`grep "^failures" $WS/test/acceptance/runtests-$mode.txt | awk '{print $3}'`
if [ "$failures" = "0" ]; then
...
#############################
# NoDebugger DebugDebugger Acceptance
#############################
export shell_name=avmshell_sd
export AVM="$WS/objdir/shell/$shell_name$shell_suffix"
export mode="release-nodebugger"
...
################################################################################
#
#   System malloc Configuration Compilation and Testing
#
################################################################################
################################################################################

##############################
# System malloc Release compilation
##############################
shell_name="avmshell_sysmalloc"
configure_args="${base_configure_args} --enable-use-system-malloc"
compiledir="objdir-release-sysmalloc"
features="${base_features} +AVMFEATURE_USE_SYSTEM_MALLOC"
unset AVM
cd $WS/build/buildbot/slaves/scripts
../all/compile-generic.sh "$rev_id" "$configure_args" "${shell_name}${shell_suffix}" "false" "$features" "$compiledir"
if [ "$?" != "0" ]; then
    echo "Release sysmalloc compilation failure."
    exitcode=1
fi
if [ -f $WS/$compiledir/shell/$shell_name$shell_suffix${shell_extension} ]; then
    mv $WS/$compiledir/shell/$shell_name$shell_suffix${shell_extension} $WS/objdir/shell/$shell_name$shell_suffix${shell_extension}
...
#############################
# System malloc Release Acceptance
#############################
export shell_name=avmshell_sysmalloc
export AVM="$WS/objdir/shell/$shell_name$shell_suffix"
export mode="release-sysmalloc"
export vmargs=""
cd $WS/build/buildbot/slaves/scripts
../all/run-acceptance-generic.sh "$rev_id" "$shell_name$shell_suffix" "$vmargs" "x64-lnx-tvm-release-use-system-malloc" "--showtimes --log runtests-$mode.txt --logjunit=acceptance-$mode.xml --threads=$threads --testtimeout=300 $suite"
failures=`grep "^failures" $WS/test/acceptance/runtests-$mode.txt | awk '{print $3}'`
if [ "$failures" = "0" ]; then
    echo "all tests passed"
else
...
if [ "$?" = "0" ]; then
    ...
    echo "error: Code coverage failed"
    exitcode=1
fi
