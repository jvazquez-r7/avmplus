...
LIns* LirHelper::atomToNative(BuiltinType bt, LIns* atom)
{
    ...
    case BUILTIN_float:
        if (atom->isImmP())
            return lirout->insImmF(AvmCore::atomToFloat((Atom)atom->immP()), atom->isTainted());
        else
            return ldf(atom, -AtomConstants::kSpecialBibopType, ACCSET_OTHER);
#endif
    case BUILTIN_number:
        if (atom->isImmP())
            return lirout->insImmD(AvmCore::number_d((Atom)atom->immP()), atom->isTainted());
        else
            return callIns(FUNCTIONID(number_d), 1, atom);

    case BUILTIN_int:
        if (atom->isImmP())
            return InsConst(AvmCore::integer_i((Atom)atom->immP()), atom->isTainted());
        else
            return callIns(FUNCTIONID(integer_i), 1, atom);

    case BUILTIN_uint:
        if (atom->isImmP())
            return InsConst(AvmCore::integer_u((Atom)atom->immP()), atom->isTainted());
        else
