...
FLOAT_ONLY(           !(v.sst_mask == (1 << SST_float)  && v.traits == FLOAT_TYPE)   &&
            ...
            CodegenDriver* driver)
    {
        ...
        if (core->config.njconfig.cseopt)
            lirout = cseFilter = new (*alloc1) CseFilter(lirout, TR_NUM_USED_ACCS, *alloc1, core->config.njconfig);
    ...
    */
    ...
    void CodegenLIR::emitCall(AbcOpcode opcode, intptr_t method_id, int argc, LIns* obj, Traits* objType, Traits* result, MethodSignaturep ms)
    {
        ...
        case OP_callinterface:
        {
            ...
            // We don't need to blind method_id, as it is a MethodInfo* pointer, not a method index.
        ...
        if (!iid) {
            ...
            // take the risk?
            out = callIns(fid, 4, target, method, InsConst(argc, /*tainted*/true), apAddr);
        } else {
            const CallInfo *fid;
            switch (rbt) {
#ifdef VMCFG_FLOAT
            ...
            out = callIns(fid, 5, target, method, InsConst(argc, /*tainted*/true), apAddr, iid);
        }
    ...
    void CodegenLIR::emitConstruct(int argc, LIns* ctor, Traits* ctraits)
    {
        ...
        LIns* newobj = callIns(FUNCTIONID(op_construct), 4, env_param, func, InsConst(argc, /*tainted*/true), args);
        liveAlloc(args);
    ...
    void CodegenLIR::emit(AbcOpcode opcode, uintptr_t op1, uintptr_t op2, Traits* result)
    {
        ...
        switch (opcode)
        {
            ...
            case OP_declocal: {
                ...
                AvmAssert(result == vt);
                if(vt == NUMBER_TYPE)
                    addIns = binaryIns(LIR_addd, localGetd(index), i2dIns(InsConst(incr, /*tainted*/false)));
                else if(vt == FLOAT_TYPE)
                    addIns = binaryIns(LIR_addf, localGetf(index), InsConstFlt((float)incr, /*tainted*/false));
                else if(vt == FLOAT4_TYPE)
                    addIns = binaryIns(LIR_addf4, localGetf4(index), Ins(LIR_f2f4, InsConstFlt((float)incr, /*tainted*/false)) );
                else {
                    // NUMERIC - i.e., unknown
            ...
            case OP_decrement_i: {
                ...
                // We do not blind the increment/decrement value, as it is always 1 or -1.
                AvmAssert(incr == 1 || incr == -1);
                localSet(index, binaryIns(LIR_addi, localGet(index), InsConst(incr, /*tainted*/false)), result);
                break;
            }
            ...
            case OP_call:
            {
                ...
                case BUILTIN_methodClosure: {
                    ...
                    LIns* call_ptr = ldp(func, offsetof(FunctionObject, m_call_ptr),
                                         ACCSET_OTHER);
                    result_ins = callIns(FUNCTIONID(call_ptr), 4, call_ptr, func, InsConst(argc, /*tainted*/true), ap);
                    break;
                }
                default:
                    // generic call
                    ...
                    ap = storeAtomArgs(loadAtomRep(funcDisp+1), argc, funcDisp+2);
                    result_ins = callIns(FUNCTIONID(op_call_atom), 4, env_param, func, InsConst(argc, /*tainted*/true), ap);
                    break;
                }
                liveAlloc(ap);
            ...
            case OP_callpropvoid:
            {
                ...
                if (AvmCore::isSlotBinding(b)) {
                    ...
                    out = callIns(FUNCTIONID(op_call_atom), 4, env_param, funcAtom, InsConst(argc, /*tainted*/true), ap);
                }
                    ...
                    out = callIns(FUNCTIONID(call_cache_handler), 6,
                                  handler, cacheAddr, base, InsConst(argc, /*tainted*/true), ap, env_param);
                }
                else {
                    // generic late bound call to anything
                    out = callIns(FUNCTIONID(callprop_late), 5, env_param, base, multi, InsConst(argc, /*tainted*/true), ap);
                    liveAlloc(multi);
                }
                liveAlloc(ap);
            ...
            case OP_constructprop:
            {
                ...
                LIns* i3 = callIns(FUNCTIONID(construct_late), 4,
                                   env_param, multi, InsConst(argc, /*tainted*/true), ap);
                liveAlloc(multi);
                liveAlloc(ap);
            ...
            case OP_applytype:
            {
                ...
                LIns* i3 = callIns(FUNCTIONID(op_applytype), 4,
                                   env_param, func, InsConst(argc, /*tainted*/true), ap);
                liveAlloc(ap);
            ...
            case OP_newarray:
            {
                ...
                LIns* ap = storeAtomArgs(argc, arg0);
                LIns* i3 = callIns(FUNCTIONID(newarray), 3, env_param, InsConst(argc, /*tainted*/true), ap);
                liveAlloc(ap);
