...
NanoStaticAssert(LIR_start == 0 && LIR_sentinel <= 256); // It's ok if LIR_sentinel is 256 since it's not actually used as opcode.
    ...
    public:
        ...
        LIns* insImmI(int32_t imm) {
            return insImmI(imm, false);
        }
#ifdef NANOJIT_64BIT
        LIns* insImmQ(uint64_t imm) {
            return insImmQ(imm, false);
        }
#endif
        LIns* insImmF(float f) {
            return insImmF(f, false);
        }
        LIns* insImmF4(const float4_t& f4) {
            return insImmF4(f4, false);
        }
        LIns* insImmD(double d) {
            return insImmD(d, false);
        }
...
#pragma warning(disable:4996) // disable "strcpy is unsafe" warnings in Visual Studio, they break the build
    ...
    public:
        ...
        LIns* insImmF(float f, bool tainted) {
            return add(out->insImmF(f, tainted));
        }
    ...
    public:
        CseFilter(LirWriter *out, uint8_t embNumUsedAccs, Allocator&, const Config& config);
