...
#undef OP___
    ...
    LIns* ExprFilter::ins2(LOpcode v, LIns* oprnd1, LIns* oprnd2)
    {
        ...
        // be counterproductive if any internally-generated expressions can trigger it.
        ...
        // Note that we taint the result of a folded operation even when only one of the
        // operands is tainted.  While the attacker presumably cannot directly control an
        // untainted operand, its value may be predictable.  In this case, even a shift count
        // under attacker control is trivially exploitable.  We can isolate and position
        // a single bit with shifts, then add/or several such expressions to construct any
        // value desired.
        ...
        if (oprnd1->isImmI() && oprnd2->isImmI()) {
            ...
            switch (v) {
#if NJ_SOFTFLOAT_SUPPORTED
            case LIR_ii2d:  return insImmD(do_join(c1, c2), tainted);
#endif
            case LIR_eqi:   return insImmI(c1 == c2, tainted);
            case LIR_lti:   return insImmI(c1 <  c2, tainted);
            case LIR_gti:   return insImmI(c1 >  c2, tainted);
            case LIR_lei:   return insImmI(c1 <= c2, tainted);
            case LIR_gei:   return insImmI(c1 >= c2, tainted);
            case LIR_ltui:  return insImmI(uint32_t(c1) <  uint32_t(c2), tainted);
            case LIR_gtui:  return insImmI(uint32_t(c1) >  uint32_t(c2), tainted);
            case LIR_leui:  return insImmI(uint32_t(c1) <= uint32_t(c2), tainted);
            case LIR_geui:  return insImmI(uint32_t(c1) >= uint32_t(c2), tainted);

            case LIR_lshi:  return insImmI(c1 << (c2 & 0x1f), tainted);
            case LIR_rshi:  return insImmI(c1 >> (c2 & 0x1f), tainted);
            case LIR_rshui: return insImmI(uint32_t(c1) >> (c2 & 0x1f), tainted);

            case LIR_ori:   return insImmI(c1 | c2, tainted);
            case LIR_andi:  return insImmI(c1 & c2, tainted);
            case LIR_xori:  return insImmI(c1 ^ c2, tainted);
            ...
            switch (v) {
            case LIR_eqq:   return insImmI(c1 == c2, tainted);
            case LIR_ltq:   return insImmI(c1 <  c2, tainted);
            case LIR_gtq:   return insImmI(c1 >  c2, tainted);
            case LIR_leq:   return insImmI(c1 <= c2, tainted);
            case LIR_geq:   return insImmI(c1 >= c2, tainted);
            case LIR_ltuq:  return insImmI(uint64_t(c1) <  uint64_t(c2), tainted);
            case LIR_gtuq:  return insImmI(uint64_t(c1) >  uint64_t(c2), tainted);
            case LIR_leuq:  return insImmI(uint64_t(c1) <= uint64_t(c2), tainted);
            case LIR_geuq:  return insImmI(uint64_t(c1) >= uint64_t(c2), tainted);
            ...
            switch (v) {
            case LIR_lshq:  return insImmQ(c1 << (c2 & 0x3f), tainted);
            case LIR_rshq:  return insImmQ(c1 >> (c2 & 0x3f), tainted);
            case LIR_rshuq: return insImmQ(uint64_t(c1) >> (c2 & 0x3f), tainted);
            ...
            bool tainted = (oprnd1->isTainted() | oprnd2->isTainted());
            switch (v) {
            case LIR_eqd:   return insImmI(c1 == c2, tainted);
            case LIR_ltd:   return insImmI(c1 <  c2, tainted);
            case LIR_gtd:   return insImmI(c1 >  c2, tainted);
            case LIR_led:   return insImmI(c1 <= c2, tainted);
            case LIR_ged:   return insImmI(c1 >= c2, tainted);

            case LIR_addd:  return insImmD(c1 + c2, tainted);
            case LIR_subd:  return insImmD(c1 - c2, tainted);
            case LIR_muld:  return insImmD(c1 * c2, tainted);
            case LIR_divd:  return insImmD(c1 / c2, tainted);
            ...
            bool tainted = (oprnd1->isTainted() | oprnd2->isTainted());
            switch (v) {
                case LIR_eqf:   return insImmI(c1 == c2, tainted);
                case LIR_ltf:   return insImmI(c1 <  c2, tainted);
                case LIR_gtf:   return insImmI(c1 >  c2, tainted);
                case LIR_lef:   return insImmI(c1 <= c2, tainted);
                case LIR_gef:   return insImmI(c1 >= c2, tainted);
                    
                case LIR_addf:  return insImmF(c1 + c2, tainted);
                case LIR_subf:  return insImmF(c1 - c2, tainted);
                case LIR_mulf:  return insImmF(c1 * c2, tainted);
                case LIR_divf:  return insImmF(c1 / c2, tainted);
            ...
            bool tainted = (oprnd1->isTainted() | oprnd2->isTainted());
            switch (v) {
                case LIR_eqf4:  return insImmI(f4_eq_i(c1, c2), tainted);
                    
                case LIR_addf4: return insImmF4(f4_add(c1, c2), tainted);
                case LIR_subf4: return insImmF4(f4_sub(c1, c2), tainted);
                case LIR_mulf4: return insImmF4(f4_mul(c1, c2), tainted);
                case LIR_divf4: return insImmF4(f4_div(c1, c2), tainted);
        ...
        if (oprnd2->isImmI()) {
            ...
            case LIR_addi:
                if (oprnd1->isop(LIR_addi) && oprnd1->oprnd2()->isImmI()) {
                    ...
                    oprnd2 = insImmI(c, (oprnd1->oprnd2()->isTainted() | oprnd2->isTainted()));
            ...
            if (c == 0) {
                ...
                case LIR_gtui: return insImmI(0, oprnd2->isTainted());   // u32 >  0xffffffff -> always false
                case LIR_leui: return insImmI(1, oprnd2->isTainted());   // u32 <= 0xffffffff -> always true
                default:       break;
                }
                ...
                if (oprnd1->isCmp()) {
                    ...
                    case LIR_gtui:  return insImmI(0, oprnd2->isTainted());  // 0or1 > 1 -> always false
                    default:        break;
                    }
                } else if (v == LIR_muli) {
                    return oprnd1;          // x * 1 = x
            ...
            if (c == 0) {
                ...
                case LIR_gtuq: return insImmI(0, oprnd2->isTainted());   // u64 >  0xffffffffffffffff -> always false
                case LIR_leuq: return insImmI(1, oprnd2->isTainted());   // u64 <= 0xffffffffffffffff -> always true
                default:       break;
                }
                ...
                if (oprnd1->isCmp()) {
                    ...
                    case LIR_gtuq:  return insImmI(0, oprnd2->isTainted());  // 0or1 > 1 -> always false
                    default:        break;
                    }
                }
            }
    ...
    char* LInsPrinter::formatRef(RefBuf* buf, LIns *ref, bool showImmValue)
    {
        ...
        if (ref->isImmI() && showImmValue) {
            VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name, formatImmI(&buf2, ref->immI()), formatTaint(ref));
        }
#ifdef NANOJIT_64BIT
        else if (ref->isImmQ() && showImmValue) {
            VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name, formatImmQ(&buf2, ref->immQ()), formatTaint(ref));
        }
#endif
        else if (ref->isImmD() && showImmValue) {
            VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name, formatImmD(&buf2, ref->immD()), formatTaint(ref));
        } 
        else if (ref->isImmF() && showImmValue) { // float is printed as a double
            VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name, formatImmD(&buf2, ref->immF()), formatTaint(ref));
        }
            ...
            VMPI_snprintf(buf->buf, buf->len, "%s/*%s,%s,%s,%s*/%s", name, formatImmD(&bufx,f4_x(v)),
                          formatImmD(&bufy,f4_y(v)), formatImmD(&bufz,f4_z(v)), formatImmD(&bufw,f4_w(v)), formatTaint(ref));
        }
        else {
    ...
    char* LInsPrinter::formatIns(InsBuf* buf, LIns* i)
    {
        ...
        {
            case LIR_immi:
                VMPI_snprintf(s, n, "%s = %s %s%s", formatRef(&b1, i, /*showImmValue*/false),
                              lirNames[op], formatImmI(&b2, i->immI()), formatTaint(i));
                break;

#ifdef NANOJIT_64BIT
            case LIR_immq:
                VMPI_snprintf(s, n, "%s = %s %s%s", formatRef(&b1, i, /*showImmValue*/false),
                              lirNames[op], formatImmQ(&b2, i->immQ()), formatTaint(i));
                break;
#endif

            case LIR_immf:
                VMPI_snprintf(s, n, "%s = %s %s%s", formatRef(&b1, i, /*showImmValue*/false),
                              lirNames[op], formatImmD(&b2, i->immF()), formatTaint(i));
                break;

            case LIR_immf4:{
                RefBuf b5;
                float4_t v = i->immF4();
                VMPI_snprintf(s, n, "%s = %s %s,%s,%s,%s%s", formatRef(&b1, i, /*showImmValue*/false),
                              lirNames[op], formatImmD(&b2, f4_x(v)), formatImmD(&b3, f4_y(v)),
                              formatImmD(&b4, f4_z(v)), formatImmD(&b5, f4_w(v)), formatTaint(i));
                break;
            }

            case LIR_immd:
                VMPI_snprintf(s, n, "%s = %s %s%s", formatRef(&b1, i, /*showImmValue*/false),
                              lirNames[op], formatImmD(&b2, i->immD()), formatTaint(i));
                break;
    ...
    CseFilter::CseFilter(LirWriter *out, uint8_t embNumUsedAccs, Allocator& alloc, const Config& config)
        : LirWriter(out),
          ...
          config(config)
    {
        ...
        m_capNL[NLImmISmall]  = 2*17;   // covers 0..16, tainted/untainted (over 50% on Tracemonkey)
    ...
    inline LIns* CseFilter::findImmF4(const float4_t& a, uint32_t &k, bool tainted)
    {
        ...
        while (true) {
            ...
            if (VMPI_memcmp(&a, &b, sizeof(float4_t)) == 0 && ins->isTainted() == tainted)
                return ins;
            k = (k + n) & bitmask;
            n += 1;
        }
    ...
    LIns* CseFilter::insImmI(int32_t imm, bool tainted)
    {
        ...
        // Each value gets two slots in m_capNL: one for tainted and another for untainted.
