...
shell_suffix=""
shell_extension=""
base_configure_args="--target=i686-darwin --mac-sdk=106"
...
cd $WS/objdir/shell/
wget ${build_url}platform=mac-intel-10_5/artifact/objdir/shell/avmshell${shell_suffix}${shell_extension}
chmod 777 avmshell${shell_suffix}${shell_extension}
wget ${build_url}platform=mac-intel-10_5/artifact/objdir/shell/avmshell_s${shell_suffix}${shell_extension}
chmod 777 avmshell_s${shell_suffix}${shell_extension}
wget ${build_url}platform=mac-intel-10_5/artifact/objdir/shell/avmshell_d${shell_suffix}${shell_extension}
chmod 777 avmshell_d${shell_suffix}${shell_extension}
wget ${build_url}platform=mac-intel-10_5/artifact/objdir/shell/avmshell_sd${shell_suffix}${shell_extension}
...
export vmargs=""
cd $WS/build/buildbot/slaves/scripts
../all/run-acceptance-generic.sh "$rev_id" "$shell_name$shell_suffix" "$vmargs" "x86-mac-tvm-releasedebugger-air" "--showtimes --log runtests-$mode.txt --logjunit=acceptance-$mode.xml --threads=$threads --testtimeout=300 $suite"
failures=`grep "^failures" $WS/test/acceptance/runtests-$mode.txt | awk '{print $3}'`
if [ "$failures" = "0" ]; then
    echo "all tests passed"
else
...
p4 sync -f ./as3/asc/import-with-float4-helper.abc
