...
namespace avmplus
{
    ...
    void ListImpl<T,ListHelper>::destroy()
    {
        ...
        if (m_data != NULL)
        {
            ...
            if (m_data->gc()->GetAttachedSampler())
                ((avmplus::IMemorySampler *)m_data->gc()->GetAttachedSampler())->recordDeallocation(this, 0);
#endif
            if (m_data->len > 0)
    ...
    void ListImpl<T,ListHelper>::set_capacity(uint32_t cap)
    {
        ...
        if (capacity() != cap)
        {
            ...
            {
                ((IMemorySampler *)gc->GetAttachedSampler())->recordObjectReallocation(this);
            }
#endif
        }
    }
