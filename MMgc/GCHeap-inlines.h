...
namespace MMgc
{
    ...
    // the object size is less than kMaxObjectSize.  The problem is that kMaxObjectSize
    // as presently defined on a 32-bit platform will already exceed available memory.
    // See GC::allocaPush().

    /* static */
    REALLY_INLINE uintptr_t GCHeap::AddToAddressWithOverflowCheck(uintptr_t base, size_t offset)
    {
