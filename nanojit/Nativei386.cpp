...
#define SIBIDX(n)    "1248"[n]
    ...
    */
    ...
    bool RegAlloc::canRemat(LIns* ins)
    {
        ...
        return (ins->isImmAny() && !(ins->isImmI() && ins->isTainted() && Assembler::shouldBlind(ins->immI()))) || ins->isop(LIR_allocp) || canRematLEA(ins);
    }
    ...
    void Assembler::asm_restore(LIns* ins, Register r)
    {
        ...
        if (ins->isop(LIR_allocp)) {
            ...
            asm_immd(r, ins->immDasQ(), ins->immD(), /*canClobberCCs*/false, ins->isTainted());
        } else if (ins->isImmF()) {
            asm_immf(r, ins->immFasI(), ins->immF(), /*canClobberCCs*/false, ins->isTainted());
        } else if (ins->isImmF4()) {
    ...
    // NOTE: Since float and double constants will almost certainly originate in user-controlled
    // literals, the attempt to optmize their treatment below will almost assuredly be ineffective
    // when constant blinding is used.
    ...
    void Assembler::asm_cmpi(LIns *cond)
    {
        ...
        if (rhs->isImmI() && !(rhs->isTainted() && shouldBlind(rhs->immI()))) {
            int c = rhs->immI();
            // findRegFor() can call asm_restore() -- asm_restore() better not disturb the CCs!
    ...
    void Assembler::asm_arith(LIns* ins)
    {
        ...
        // Second special case.
        if (op == LIR_addi && lhs->isop(LIR_allocp) && rhs->isImmI() && !rhs->isTainted()) {
        ...
        case LIR_rshi:
        case LIR_rshui:
            isConstRhs = rhs->isImmI() && !(rhs->isTainted() && shouldBlind(rhs->immI()));
            if (!isConstRhs) {
        ...
        if (!isConstRhs) {
            ...
            int c = rhs->immI();
            switch (op) {
            case LIR_lshi:       NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI())); SHLi(rr, c);    break;
            case LIR_rshi:       NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI())); SARi(rr, c);    break;
            case LIR_rshui:      NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI())); SHRi(rr, c);    break;
            case LIR_addjovi:
            case LIR_addxovi:    NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI())); ADDi(rr, c);    break;
            case LIR_subjovi:
            case LIR_subxovi:    NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI())); SUBi(rr, c);    break;
    ...
    void Assembler::asm_load32(LIns* ins)
    {
        ...
        if (base->isImmI()) {
            // We assume that the base address is never tainted.
            NanoAssert(!base->isTainted());
    ...
    void Assembler::asm_cmov(LIns* ins)
    {
        ...
        if (!_config.i386_sse2 && (ins->isop(LIR_cmovd) || ins->isop(LIR_cmovf))) {
            ...
            if (iffalse->isImmD()) {
                asm_immd(FST0, iffalse->immDasQ(), iffalse->immD(), /*canClobberCCs*/false, iffalse->isTainted());
            } else if (iffalse->isImmF()) {


                asm_immf(FST0, iffalse->immFasI(), iffalse->immF(), /*canClobberCCs*/false, iffalse->isTainted());
            } else {
    ...
    void Assembler::asm_immi(Register r, int32_t val, bool canClobberCCs, bool blind)
    {
    ...
    void Assembler::asm_immf(Register r, int32_t i, float f, bool canClobberCCs, bool blind)
    {
        ...
        // Special double values (zero, one) and those loaded from pools need not be blinded.
    ...
    void Assembler::asm_immf(LIns* ins)
    {
        ...
        if (ins->isInReg()) {
            ...
            asm_immf(rr, ins->immFasI(), ins->immF(), /*canClobberCCs*/true, ins->isTainted());
        } else {
    ...
    void Assembler::asm_immf4(Register r, const float4_t& f4, bool canClobberCCs, bool blind){
    ...
    void Assembler::asm_immd(Register r, uint64_t q, double d, bool canClobberCCs, bool blind)
    {
    ...
    void Assembler::asm_immd(LIns* ins)
    {
        ...
        if (ins->isInReg()) {
            Register rr = ins->getReg();
            NanoAssert(rmask(rr) & FpRegs);
            asm_immd(rr, ins->immDasQ(), ins->immD(), /*canClobberCCs*/true, ins->isTainted());
        } else {
            // Do nothing, will be rematerialized when necessary.
        }
